...
import com.example.java8.streamex.Internals.CloneableSpliterator;
...
interface TakeDrop {
    ...
    final class TDOfInt extends AbstractIntSpliterator implements IntConsumer {
        ...
        TDOfInt(OfInt source, boolean drop, boolean inclusive, IntPredicate predicate) {
    ...
    final class TDOfLong extends AbstractLongSpliterator implements LongConsumer {
        ...
        TDOfLong(OfLong source, boolean drop, boolean inclusive, LongPredicate predicate) {
            super(source.estimateSize(), source.characteristics() & (ORDERED | SORTED | CONCURRENT | IMMUTABLE | NONNULL
                | DISTINCT));
            this.drop = drop;
            this.predicate = predicate;
    ...
    final class TDOfDouble extends AbstractDoubleSpliterator implements DoubleConsumer {
        ...
        private final boolean inclusive;
        private boolean checked;
        private final OfDouble source;
        private double cur;

        TDOfDouble(OfDouble source, boolean drop, boolean inclusive, DoublePredicate predicate) {
            super(source.estimateSize(), source.characteristics() & (ORDERED | SORTED | CONCURRENT | IMMUTABLE | NONNULL
                | DISTINCT));
            this.drop = drop;
            this.predicate = predicate;
