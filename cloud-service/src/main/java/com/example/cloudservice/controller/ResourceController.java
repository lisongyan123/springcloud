...
import com.example.cloudservice.domain.vo.MenuTreeNode;
...
import static com.example.cloudservice.common.helper.TreeHepler.buildTreeBy2Loop;
...
//    @ApiOperation(value = "获取用户被授权菜单",notes = "通过uid获取对应用户被授权的菜单列表,获取完整菜单树形结构")
    ...
    public Message getAuthorityMenu(HttpServletRequest request) throws Exception {
        ...
        List<MenuTreeNode> menuTreeNodes = buildTreeBy2Loop(treeNodes,-1);
...
//    @ApiOperation(value = "获取API list", notes = "需要分页,根据teamId判断,-1->获取api分类,0->获取全部api,id->获取对应分类id下的api",httpMethod = "GET")
    ...
    public Message getApiList(@PathVariable Integer teamId, @PathVariable Integer currentPage, @PathVariable Integer pageSize) {
        ...
        if (teamId == -1) {
            // -1 为获取api分类
            resources = resourceService.getApiTeamList(TYPE3);
            return new Message().ok(6666,"return apis success").addData("data",resources);
        }
        PageHelper.startPage(currentPage, pageSize);
        if (teamId == 0) {
            // 0 为获取全部api
            resources = resourceService.getApiTeamList(TYPE2);
        } else {
