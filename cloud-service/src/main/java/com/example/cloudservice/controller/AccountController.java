...
import com.example.cloudservice.config.shiro.support.factory.LogTaskFactory;
import com.example.cloudservice.config.shiro.support.manager.LogExeManager;
...
import static com.example.cloudservice.common.helper.RequestHelper.getRequestBodyMap;
import static com.example.cloudservice.common.helper.TokenHelper.issueJWT;
...
public class AccountController extends BaseAction {
    ...
    public Message accountLogin(HttpServletRequest request) throws Exception {
        ...
        String jwt = issueJWT(UUID.randomUUID().toString(), appId,
        ...
        LogExeManager.getInstance().executeLogTask(LogTaskFactory.loginLog(appId, getIpAddr(WebUtils.toHttp(request)), (short) 1, "登录成功"));
    ...
    public Message accountRegister(HttpServletRequest request) {
        ...
        if (isEncryptPassword) {
            ...
            String tokenKey = redisTemplate.opsForValue()
                    .get("TOKEN_KEY_" + getIpAddr(WebUtils.toHttp(request)).toUpperCase()+userKey);
            password = aesDecode(password, tokenKey);
        }
        String salt = getRandomString(6);
        // 存储到数据库的密码为 MD5(原密码+盐值)
        authUser.setPassword(md5(password + salt));
        ...
        if (accountService.registerAccount(authUser)) {
            LogExeManager.getInstance().executeLogTask(LogTaskFactory.registerLog(uid, getIpAddr(WebUtils.toHttp(request)), (short) 1, "注册成功"));
            return new Message().ok(2002, "注册成功");
        } else {
            LogExeManager.getInstance().executeLogTask(LogTaskFactory.registerLog(uid, getIpAddr(WebUtils.toHttp(request)), (short) 0, "注册失败"));
