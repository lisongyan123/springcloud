...
public class BonJwtFilter extends AbstractPathMatchingFilter {
            ...
            try {
                ...
                if (STR_EXPIRED.equals(e.getMessage())) {
                    ...
                    if (null != refreshJwt && refreshJwt.equals(jwt)) {
                        ...
                        String newJwt = issueJWT(UUID.randomUUID().toString(),appId,
                        ...
                        responseWrite(JSON.toJSONString(message),servletResponse);
                        return false;
                    }else {
                ...
                // 其他错误
                LOGGER.error(getIpAddr(WebUtils.toHttp(servletRequest))+"--JWT认证失败"+e.getMessage(),e);
    ...
    private boolean isJwtSubmission(ServletRequest request) {
        ...
        String jwt = getHeader(request,"authorization");
        String appId = getHeader(request,"appId");
    ...
    private AuthenticationToken createJwtToken(ServletRequest request) {
        ...
        Map<String,String> maps = getRequestHeaders(request);
